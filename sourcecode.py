...
def generate():
    ...
    for bar in range(1, 60):
        ...
        bar = canvas.create_rectangle(barstart, randomY, barend, 365, fill='#1BCA9B')
        barList.append(bar)
        barstart += 10
        barend += 10
    ...
    for i in range(len(lengthList) - 1):
        if lengthList[i] == min(lengthList):
            canvas.itemconfig(barList[i], fill='#487EB0')
        elif lengthList[i] == max(lengthList):
            canvas.itemconfig(barList[i], fill='#192A56')
...
canvas.grid(column=0, row=0, columnspan=500)

# Buttons

insert = tk.Button(window, text='Insertion Sort', command=insertion_sort,bg="#7CEC9F")
select = tk.Button(window, text='Selection Sort', command=selection_sort,bg="#7CEC9F")
bubble = tk.Button(window, text='Bubble Sort', command=bubble_sort,bg="#7CEC9F")
shuf = tk.Button(window, text='Shuffle', command=generate,bg="#7CEC9F")
